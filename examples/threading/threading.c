...
void *threadfunc(void *thread_param)
{
    ...
    DEBUG_LOG("waiting for %d ms before trying to obtain mutex", thread_args->wait_to_obtain_ms);
    sleep(thread_args->wait_to_obtain_ms);

    int rc = pthread_mutex_lock(thread_args->mutex);
    if (rc != 0)
    {
        ERROR_LOG("unable to acquire mutex, with return code: %d", rc);
        thread_args->thread_complete_success = false;
        return thread_param;
    }

    DEBUG_LOG("waiting for %d ms to release mutex", thread_args->wait_to_release_ms);
    sleep(thread_args->wait_to_release_ms);

    rc = pthread_mutex_unlock(thread_args->mutex);
    if (rc != 0)
    {
        ERROR_LOG("unable to release mutex, with retur code: %d", rc);
        thread_args->thread_complete_success = false;
        return thread_param;
    }

    DEBUG_LOG("all done, exiting now");
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex, int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    thread_data->wait_to_obtain_ms = wait_to_obtain_ms / 1000;
    thread_data->wait_to_release_ms = wait_to_release_ms / 1000;

    int rc = pthread_create(thread, NULL, &threadfunc, thread_data);
    if (rc != 0)
    {
        ERROR_LOG("unable to create thread with rc %d", rc);
        return false;
    }

    DEBUG_LOG("created a new thread with the following args [thread ID, wait to obtain, wait to release]: [%lu, %d, %d]",
              (unsigned long)*thread, wait_to_obtain_ms, wait_to_obtain_ms);
